			...
			<div class="content" slot="content">
				...
				<div class="table-content">
					...
					<el-table :data="detail" height="442" border style="width:100%">
						...
						<el-table-column prop="settlementTypeName" label="结算方式" min-width="80"></el-table-column>
						<el-table-column prop="settlementAccountName" label="户名" min-width="80"></el-table-column>
						<el-table-column prop="settlementAccountNumber" label="账号" min-width="80"></el-table-column>
					</el-table>
	...
	export default {
		data() {
			var crumbs = [
				...
				{path: '/reports/settlement/settlementDetail', name: '结算对象详情'},
			];
			return {
				crumbs,
				formSearch: {
		...
		methods: {
            ...
            handleExport(){
                utils.export('/pms/report/settlement/list/detail/export.do',{"id":this.$route.query.id,"settlementReceiver": this.$route.query.settlementReceiver,"filter":"","startTime":this.formSearch.date.length >0 && this.formSearch.date[0]?moment(this.formSearch.date[0]).format('YYYY-MM-DD'):'',"endTime":this.formSearch.date.length >1 && this.formSearch.date[1]?moment(this.formSearch.date[1]).format('YYYY-MM-DD'):''})
            }
		},
		created(){
			this.refresh()
